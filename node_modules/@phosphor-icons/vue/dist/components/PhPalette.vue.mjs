import { defineComponent as M, inject as i, computed as h, openBlock as o, createElementBlock as e, mergeProps as g, unref as t, renderSlot as u, createCommentVNode as y, createElementVNode as a } from "vue";
const f = ["width", "height", "fill", "transform"], v = { key: 0 }, w = /* @__PURE__ */ a("path", { d: "M203.6,51A108,108,0,1,0,92,229.9a38.2,38.2,0,0,0,12,2,36,36,0,0,0,36-36V192a12,12,0,0,1,12-12h46.2a35.8,35.8,0,0,0,35.1-28,103.9,103.9,0,0,0,2.7-24.9A107.3,107.3,0,0,0,203.6,51Zm6.3,95.7h0a11.8,11.8,0,0,1-11.7,9.3H152a36,36,0,0,0-36,36v3.9a11.9,11.9,0,0,1-5.1,9.8,11.8,11.8,0,0,1-10.9,1.5A84,84,0,0,1,127.1,44h.9a84.3,84.3,0,0,1,84,83.3A79,79,0,0,1,209.9,146.7ZM144,76a16,16,0,1,1-16-16A16,16,0,0,1,144,76ZM96.8,110A16,16,0,1,1,91,88.1,16,16,0,0,1,96.8,110Zm0,36A16,16,0,1,1,75,140.1,16.1,16.1,0,0,1,96.8,146Zm90.1-52A16,16,0,1,1,165,88.1,16,16,0,0,1,186.9,94Z" }, null, -1), k = [
  w
], V = { key: 1 }, x = /* @__PURE__ */ a("path", {
  d: "M224,127.2a96.2,96.2,0,0,1-2.4,22.2A24,24,0,0,1,198.2,168H152a23.9,23.9,0,0,0-24,24v3.9a23.9,23.9,0,0,1-32,22.6A96,96,0,0,1,127,32C179.8,31.5,223.6,74.3,224,127.2Z",
  opacity: "0.2"
}, null, -1), C = /* @__PURE__ */ a("path", { d: "M200.8,53.9A103.4,103.4,0,0,0,128,24h-1.1A104,104,0,0,0,93.4,226.1,32,32,0,0,0,136,195.9V192a16,16,0,0,1,16-16h46.2a31.7,31.7,0,0,0,31.2-24.9,101.5,101.5,0,0,0,2.6-24A102.9,102.9,0,0,0,200.8,53.9Zm13,93.7A15.9,15.9,0,0,1,198.2,160H152a32.1,32.1,0,0,0-32,32v3.9A16,16,0,0,1,98.7,211,88.2,88.2,0,0,1,40,128a88.1,88.1,0,0,1,87.1-88h.9a88.3,88.3,0,0,1,88,87.2A86.8,86.8,0,0,1,213.8,147.6ZM140,76a12,12,0,1,1-12-12A12,12,0,0,1,140,76ZM93.4,108A12,12,0,1,1,89,91.6,12.1,12.1,0,0,1,93.4,108Zm0,40A12,12,0,1,1,77,143.6,12.1,12.1,0,0,1,93.4,148Zm90-52A12,12,0,1,1,167,91.6,12,12,0,0,1,183.4,96Z" }, null, -1), H = [
  x,
  C
], S = { key: 2 }, z = /* @__PURE__ */ a("path", { d: "M200.8,53.9A103.4,103.4,0,0,0,128,24h-1.1A104,104,0,0,0,93.4,226.1,32,32,0,0,0,136,195.9V192a16,16,0,0,1,16-16h46.2a31.7,31.7,0,0,0,31.2-24.9,101.5,101.5,0,0,0,2.6-24A102.9,102.9,0,0,0,200.8,53.9ZM89,164.4A12,12,0,1,1,93.4,148,12,12,0,0,1,89,164.4ZM93.4,108A12,12,0,1,1,89,91.6,12.1,12.1,0,0,1,93.4,108ZM128,88a12,12,0,1,1,12-12A12,12,0,0,1,128,88Zm51,24.4A12,12,0,1,1,183.4,96,12.1,12.1,0,0,1,179,112.4Z" }, null, -1), B = [
  z
], N = { key: 3 }, P = /* @__PURE__ */ a("path", { d: "M199.4,55.3A101.5,101.5,0,0,0,128,26h-1A102,102,0,0,0,94,224.2a30,30,0,0,0,40-28.3V192a18.1,18.1,0,0,1,18-18h46.2a30,30,0,0,0,29.3-23.3,103.4,103.4,0,0,0,2.5-23.6A101.3,101.3,0,0,0,199.4,55.3ZM215.8,148a17.9,17.9,0,0,1-17.6,14H152a30.1,30.1,0,0,0-30,30v3.9a18,18,0,0,1-24,17A90,90,0,0,1,64.1,64.7a88.8,88.8,0,0,1,63-26.7h.9a90.3,90.3,0,0,1,90,89.2A90.8,90.8,0,0,1,215.8,148ZM138,76a10,10,0,1,1-10-10A10,10,0,0,1,138,76ZM91.6,107A10,10,0,1,1,74.3,97a10,10,0,1,1,17.3,10Zm0,42A10,10,0,1,1,78,145.3,10.1,10.1,0,0,1,91.6,149Zm90.1-52A10,10,0,1,1,168,93.3,10,10,0,0,1,181.7,97Z" }, null, -1), b = [
  P
], E = { key: 4 }, W = /* @__PURE__ */ a("path", { d: "M200.8,53.9A103.4,103.4,0,0,0,128,24h-1.1A104,104,0,0,0,93.4,226.1,32,32,0,0,0,136,195.9V192a16,16,0,0,1,16-16h46.2a31.7,31.7,0,0,0,31.2-24.9,101.5,101.5,0,0,0,2.6-24A102.9,102.9,0,0,0,200.8,53.9Zm13,93.7A15.9,15.9,0,0,1,198.2,160H152a32.1,32.1,0,0,0-32,32v3.9A16,16,0,0,1,98.7,211,88.2,88.2,0,0,1,40,128a88.1,88.1,0,0,1,87.1-88h.9a88.3,88.3,0,0,1,88,87.2A86.8,86.8,0,0,1,213.8,147.6ZM140,76a12,12,0,1,1-12-12A12,12,0,0,1,140,76ZM93.4,108A12,12,0,1,1,89,91.6,12.1,12.1,0,0,1,93.4,108Zm0,40A12,12,0,1,1,77,143.6,12.1,12.1,0,0,1,93.4,148Zm90-52A12,12,0,1,1,167,91.6,12,12,0,0,1,183.4,96Z" }, null, -1), $ = [
  W
], j = { key: 5 }, q = /* @__PURE__ */ a("path", { d: "M198,56.7A99.7,99.7,0,0,0,128,28h-1A100,100,0,0,0,94.7,222.3a28.1,28.1,0,0,0,25.5-3.5A27.8,27.8,0,0,0,132,195.9V192a20.1,20.1,0,0,1,20-20h46.2a27.7,27.7,0,0,0,27.3-21.8A99.7,99.7,0,0,0,198,56.7Zm19.7,91.8A19.9,19.9,0,0,1,198.2,164H152a28.1,28.1,0,0,0-28,28v3.9a20.1,20.1,0,0,1-26.6,18.9A92,92,0,0,1,62.6,63.3,91.2,91.2,0,0,1,127.1,36h.9a92.4,92.4,0,0,1,92,91.2A89.7,89.7,0,0,1,217.7,148.5ZM136,76a8,8,0,1,1-8-8A8,8,0,0,1,136,76ZM89.9,106A8,8,0,0,1,76,98a8.1,8.1,0,0,1,11-2.9A8,8,0,0,1,89.9,106Zm0,44A8,8,0,0,1,87,160.9,8.1,8.1,0,0,1,76,158a8,8,0,0,1,13.9-8ZM180,98a8,8,0,1,1-11-2.9A8.1,8.1,0,0,1,180,98Z" }, null, -1), D = [
  q
], F = {
  name: "PhPalette"
}, J = /* @__PURE__ */ M({
  ...F,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(d) {
    const n = d, c = i("weight", "regular"), l = i("size", "1em"), _ = i("color", "currentColor"), Z = i("mirrored", !1), s = h(() => n.weight ?? c), r = h(() => n.size ?? l), m = h(() => n.color ?? _), p = h(() => n.mirrored !== void 0 ? n.mirrored ? "scale(-1, 1)" : void 0 : Z ? "scale(-1, 1)" : void 0);
    return (A, G) => (o(), e("svg", g({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: t(r),
      height: t(r),
      fill: t(m),
      transform: t(p)
    }, A.$attrs), [
      u(A.$slots, "default"),
      t(s) === "bold" ? (o(), e("g", v, k)) : t(s) === "duotone" ? (o(), e("g", V, H)) : t(s) === "fill" ? (o(), e("g", S, B)) : t(s) === "light" ? (o(), e("g", N, b)) : t(s) === "regular" ? (o(), e("g", E, $)) : t(s) === "thin" ? (o(), e("g", j, D)) : y("", !0)
    ], 16, f));
  }
});
export {
  J as default
};
