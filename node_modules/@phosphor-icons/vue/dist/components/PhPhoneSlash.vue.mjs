import { defineComponent as g, inject as n, computed as i, openBlock as e, createElementBlock as a, mergeProps as u, unref as t, renderSlot as Z, createCommentVNode as y, createElementVNode as o } from "vue";
const f = ["width", "height", "fill", "transform"], M = { key: 0 }, w = /* @__PURE__ */ o("path", { d: "M123.8,105.6h0l-31-34.1a.1.1,0,0,1-.1-.1L56.9,31.9A12,12,0,0,0,39.1,48.1L63.4,74.8a147.4,147.4,0,0,0-40.1,28.5,60.2,60.2,0,0,0-5,79.2,20,20,0,0,0,15.8,7.7,21.1,21.1,0,0,0,7.4-1.4l47.3-19A19.9,19.9,0,0,0,101,155.2l5.5-27.2a33.1,33.1,0,0,1,4.2-1.2l88.4,97.3a12.1,12.1,0,0,0,17,.8,12.1,12.1,0,0,0,.8-17ZM83.4,121l-5.5,27.4L35.5,165.3a36.1,36.1,0,0,1,4.8-45A123.8,123.8,0,0,1,80.4,93.5l12.7,14A20,20,0,0,0,83.4,121Zm154.3,61.5a22.4,22.4,0,0,1-4.5,4.2,12,12,0,0,1-14.1-19.4,36.1,36.1,0,0,0-3.4-47A124.1,124.1,0,0,0,130.9,84a12,12,0,0,1,.3-24h.3a147.7,147.7,0,0,1,101.2,43.3A60.2,60.2,0,0,1,237.7,182.5Z" }, null, -1), k = [
  w
], S = { key: 1 }, v = /* @__PURE__ */ o("g", { opacity: "0.2" }, [
  /* @__PURE__ */ o("path", { d: "M155.4,117.3a83.9,83.9,0,0,0-55.1.1,8,8,0,0,0-5.2,5.9l-5.8,29.5a8.2,8.2,0,0,1-4.9,5.9L37.1,177.6a7.9,7.9,0,0,1-9.3-2.5,48,48,0,0,1,4-63.3,136.1,136.1,0,0,1,192.4,0,48,48,0,0,1,4,63.3,7.9,7.9,0,0,1-9.3,2.5l-47.3-18.9a8.2,8.2,0,0,1-4.9-5.8l-6.2-29.7A7.9,7.9,0,0,0,155.4,117.3Z" })
], -1), x = /* @__PURE__ */ o("path", { d: "M120.9,108.3h0L89.8,74h-.1L53.9,34.6A8,8,0,0,0,42.1,45.4L70,76.1a143.9,143.9,0,0,0-43.8,30.1,56.1,56.1,0,0,0-4.7,73.9,16.2,16.2,0,0,0,12.6,6.1,17.1,17.1,0,0,0,5.9-1.1l47.4-19a16,16,0,0,0,9.7-11.7l5.9-29.5a72.1,72.1,0,0,1,9.1-2.5l90,99A8,8,0,0,0,208,224a8.2,8.2,0,0,0,5.4-2.1,7.9,7.9,0,0,0,.5-11.3Zm-23.3,1.5a16.1,16.1,0,0,0-10.3,12l-5.9,29.5L34.1,170.2a40,40,0,0,1,3.4-52.7,126.8,126.8,0,0,1,44-28.8L100,109.1Z" }, null, -1), z = /* @__PURE__ */ o("path", { d: "M229.8,106.2a144.4,144.4,0,0,0-109.7-42,8,8,0,1,0,.9,16,128,128,0,0,1,97.5,37.3,40,40,0,0,1,3.4,52.7l-4-1.6a8,8,0,0,0-6,14.8l4.1,1.7a17.1,17.1,0,0,0,5.9,1.1,16.2,16.2,0,0,0,12.6-6.1A56.1,56.1,0,0,0,229.8,106.2Z" }, null, -1), C = [
  v,
  x,
  z
], B = { key: 2 }, N = /* @__PURE__ */ o("path", { d: "M213.9,210.6a7.9,7.9,0,0,1-.5,11.3A8.2,8.2,0,0,1,208,224a8,8,0,0,1-5.9-2.6l-90-99a72.1,72.1,0,0,0-9.1,2.5l-5.9,29.5a16,16,0,0,1-9.7,11.7L40,185.1a17.1,17.1,0,0,1-5.9,1.1,16.2,16.2,0,0,1-12.6-6.1,56.1,56.1,0,0,1,4.7-73.9A143.9,143.9,0,0,1,70,76.1L42.1,45.4A8,8,0,0,1,53.9,34.6L89.7,74h.1l31,34.2h.1Zm15.9-104.4a144.4,144.4,0,0,0-109.7-42,8,8,0,0,0-5.5,13.4L209,181.4a7.2,7.2,0,0,0,2.9,2l4.1,1.7a17.1,17.1,0,0,0,5.9,1.1,16.2,16.2,0,0,0,12.6-6.1A56.1,56.1,0,0,0,229.8,106.2Z" }, null, -1), P = [
  N
], b = { key: 3 }, E = /* @__PURE__ */ o("path", { d: "M88.3,75.4h-.1L52.4,36a5.9,5.9,0,0,0-8.8,8L73.4,76.9a140.8,140.8,0,0,0-45.8,30.7,54,54,0,0,0-4.5,71.2,13.9,13.9,0,0,0,16.2,4.4l47.3-18.9A13.9,13.9,0,0,0,95.1,154l5.9-29.5a2.3,2.3,0,0,1,1.2-1.5,78.8,78.8,0,0,1,10.6-2.8L203.6,220a5.9,5.9,0,0,0,4.4,2,5.7,5.7,0,0,0,4-1.6,5.8,5.8,0,0,0,.4-8.4Zm10,36.3a13.8,13.8,0,0,0-9,10.5l-5.9,29.5a2,2,0,0,1-1.3,1.4l-47.3,19a2.2,2.2,0,0,1-2.3-.6,42,42,0,0,1,3.6-55.4A128.3,128.3,0,0,1,82,86.4l21.6,23.7Zm134.6,67.1a13.9,13.9,0,0,1-16.2,4.4l-4-1.6a6,6,0,1,1,4.4-11.2l4.1,1.7a2.2,2.2,0,0,0,2.3-.6,42,42,0,0,0-3.6-55.4,130.2,130.2,0,0,0-99-37.9,6,6,0,1,1-.7-12,142.2,142.2,0,0,1,108.2,41.4A54,54,0,0,1,232.9,178.8Z" }, null, -1), V = [
  E
], W = { key: 4 }, $ = /* @__PURE__ */ o("path", { d: "M120.9,108.3h-.1L89.8,74h-.1L53.9,34.6A8,8,0,0,0,42.1,45.4L70,76.1a143.9,143.9,0,0,0-43.8,30.1,56.1,56.1,0,0,0-4.7,73.9,16.2,16.2,0,0,0,12.6,6.1,17.1,17.1,0,0,0,5.9-1.1l47.4-19a16,16,0,0,0,9.7-11.7l5.9-29.5a72.1,72.1,0,0,1,9.1-2.5l90,99A8,8,0,0,0,208,224a8.2,8.2,0,0,0,5.4-2.1,7.9,7.9,0,0,0,.5-11.3Zm-23.3,1.5a16.1,16.1,0,0,0-10.3,12l-5.9,29.5L34.1,170.2a40,40,0,0,1,3.4-52.7,126.8,126.8,0,0,1,44-28.8L100,109.1Zm136.9,70.3a16.2,16.2,0,0,1-12.6,6.1,17.1,17.1,0,0,1-5.9-1.1l-4.1-1.7a8,8,0,0,1,6-14.8l4,1.6a40,40,0,0,0-3.4-52.7A128,128,0,0,0,121,80.2a8,8,0,1,1-.9-16,144.4,144.4,0,0,1,109.7,42A56.1,56.1,0,0,1,234.5,180.1Z" }, null, -1), j = [
  $
], q = { key: 5 }, D = /* @__PURE__ */ o("path", { d: "M117.9,111h0L86.8,76.7h0L51,37.3a4,4,0,1,0-6,5.4l31.8,35A138.6,138.6,0,0,0,29,109a52.1,52.1,0,0,0-4.4,68.6,12,12,0,0,0,14,3.7l47.3-18.9a12.1,12.1,0,0,0,7.3-8.8l5.9-29.5a3.6,3.6,0,0,1,2.5-2.9,83.7,83.7,0,0,1,11.9-3.2L205,218.7a4.1,4.1,0,0,0,5.7.3,4.2,4.2,0,0,0,.3-5.7Zm-19,2.6a12.2,12.2,0,0,0-7.7,8.9l-5.9,29.6a3.7,3.7,0,0,1-2.4,2.9L35.6,173.9a3.8,3.8,0,0,1-4.6-1.2,43.9,43.9,0,0,1,3.7-58A129.5,129.5,0,0,1,82.6,84l24.7,27.2A66.2,66.2,0,0,0,98.9,113.6Zm132.5,64a12,12,0,0,1-14,3.7l-4-1.6a3.9,3.9,0,0,1-2.2-5.2,4,4,0,0,1,5.2-2.2l4,1.6a3.8,3.8,0,0,0,4.6-1.2,43.9,43.9,0,0,0-3.7-58A132,132,0,0,0,120.7,76.2a4.1,4.1,0,0,1-4.2-3.8,4,4,0,0,1,3.8-4.2A140.2,140.2,0,0,1,227,109,52.1,52.1,0,0,1,231.4,177.6Z" }, null, -1), F = [
  D
], G = {
  name: "PhPhoneSlash"
}, J = /* @__PURE__ */ g({
  ...G,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(d) {
    const s = d, c = n("weight", "regular"), _ = n("size", "1em"), m = n("color", "currentColor"), p = n("mirrored", !1), l = i(() => s.weight ?? c), h = i(() => s.size ?? _), A = i(() => s.color ?? m), L = i(() => s.mirrored !== void 0 ? s.mirrored ? "scale(-1, 1)" : void 0 : p ? "scale(-1, 1)" : void 0);
    return (r, H) => (e(), a("svg", u({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: t(h),
      height: t(h),
      fill: t(A),
      transform: t(L)
    }, r.$attrs), [
      Z(r.$slots, "default"),
      t(l) === "bold" ? (e(), a("g", M, k)) : t(l) === "duotone" ? (e(), a("g", S, C)) : t(l) === "fill" ? (e(), a("g", B, P)) : t(l) === "light" ? (e(), a("g", b, V)) : t(l) === "regular" ? (e(), a("g", W, j)) : t(l) === "thin" ? (e(), a("g", q, F)) : y("", !0)
    ], 16, f));
  }
});
export {
  J as default
};
