import { defineComponent as u, inject as a, computed as i, openBlock as e, createElementBlock as c, mergeProps as A, unref as t, renderSlot as V, createCommentVNode as Z, createElementVNode as o } from "vue";
const y = ["width", "height", "fill", "transform"], M = { key: 0 }, f = /* @__PURE__ */ o("path", { d: "M234.6,47.9C230.3,42.5,222.3,36,208,36c-17.7,0-40.2,11.8-60,31.5-2.9,2.8-5.5,5.6-8,8.5V56a12,12,0,0,0-24,0V76c-2.5-2.9-5.1-5.7-8-8.5C88.2,47.8,65.7,36,48,36c-14.3,0-22.3,6.5-26.6,11.9-13.2,16.9-8.3,46-1,75,5.5,22.2,17,32.7,27.8,37.5A47.5,47.5,0,0,0,44,180a48,48,0,0,0,84,31.7,48,48,0,0,0,79.8-51.3c10.8-4.8,22.3-15.3,27.8-37.5C242.9,93.9,247.8,64.8,234.6,47.9ZM92,204a24,24,0,0,1-2.7-47.9,11.9,11.9,0,0,0,10.6-13.2,12,12,0,0,0-13.2-10.6,46.9,46.9,0,0,0-21.8,8.1c-7.8-.7-16.8-5.7-21.3-23.3-7.3-29.5-8.4-47.8-3.2-54.4.6-.8,2-2.7,7.6-2.7,11,0,28.4,9.9,43.1,24.5S116,116.6,116,128v52A24.1,24.1,0,0,1,92,204Zm120.4-86.9c-4.5,17.6-13.5,22.6-21.3,23.3a46.9,46.9,0,0,0-21.8-8.1,12,12,0,1,0-2.6,23.8A24,24,0,1,1,140,180V128c0-11.4,9.8-28.5,24.9-43.5S197,60,208,60c5.6,0,7,1.9,7.6,2.7C220.8,69.3,219.7,87.6,212.4,117.1Z" }, null, -1), S = [
  f
], v = { key: 1 }, w = /* @__PURE__ */ o("path", {
  d: "M224,120c-7.8,31.1-29,32.8-36.9,32.4h0A36,36,0,1,1,128,180a36,36,0,1,1-59.1-27.6h0c-7.9.4-29.1-1.3-36.9-32.4S17.1,48,48,48s80,48,80,80c0-32,49.1-80,80-80S232,88,224,120Z",
  opacity: "0.2"
}, null, -1), k = /* @__PURE__ */ o("path", { d: "M231.4,50.4C227.7,45.6,220.7,40,208,40c-16.7,0-38.1,11.3-57.2,30.3A144.2,144.2,0,0,0,136,87.5V56a8,8,0,0,0-16,0V87.5a144.2,144.2,0,0,0-14.8-17.2C86.1,51.3,64.7,40,48,40c-12.7,0-19.7,5.6-23.4,10.4-6.8,8.7-12.2,24.1-.4,71.5,5.9,23.4,18.7,32.7,29.5,36.3A44.8,44.8,0,0,0,48,180a44,44,0,0,0,80,25.3A44,44,0,0,0,208,180a44.8,44.8,0,0,0-5.7-21.8c10.8-3.6,23.6-12.9,29.5-36.3C238.4,95.3,243.6,66,231.4,50.4ZM92,208a28,28,0,0,1-18.3-49.2l.7-.6a28.1,28.1,0,0,1,14.5-6,8.1,8.1,0,0,0,7.1-8.9,8,8,0,0,0-8.9-7,42.9,42.9,0,0,0-21,8.1c-8.4-.2-20.9-4.5-26.3-26.3-2.9-11.5-11.6-46.3-2.6-57.9C38.6,58.4,41.3,56,48,56c26.9,0,72,45.1,72,72v52A28.1,28.1,0,0,1,92,208Zm124.2-89.9c-5.4,21.8-17.9,26.1-26.3,26.3a42.9,42.9,0,0,0-21-8.1,8,8,0,0,0-8.9,7,8.1,8.1,0,0,0,7.1,8.9,28.1,28.1,0,0,1,14.5,6l.7.6A28,28,0,1,1,136,180V128c0-26.9,45.1-72,72-72,6.7,0,9.4,2.4,10.8,4.2C227.8,71.8,219.1,106.6,216.2,118.1Z" }, null, -1), x = [
  w,
  k
], z = { key: 2 }, B = /* @__PURE__ */ o("path", { d: "M136,87.5V56a8,8,0,0,0-16,0V87.5a98.7,98.7,0,0,1,8,12.7A98.7,98.7,0,0,1,136,87.5Z" }, null, -1), N = /* @__PURE__ */ o("path", { d: "M231.4,50.4C227.7,45.6,220.7,40,208,40c-16.7,0-38.1,11.3-57.2,30.3A144.2,144.2,0,0,0,136,87.5V172a8,8,0,0,1-16,0V87.5a144.2,144.2,0,0,0-14.8-17.2C86.1,51.3,64.7,40,48,40c-12.7,0-19.7,5.6-23.4,10.4-6.8,8.7-12.2,24.1-.4,71.5,6.6,26.3,22,34.9,33.5,37.5a40,40,0,1,0,70.3,38,40,40,0,1,0,70.3-38c11.5-2.6,26.9-11.2,33.5-37.5S243.6,66,231.4,50.4Z" }, null, -1), b = [
  B,
  N
], E = { key: 3 }, P = /* @__PURE__ */ o("path", { d: "M229.8,51.6C224.9,45.2,217.5,42,208,42c-16.2,0-37,11.1-55.8,29.7A133.5,133.5,0,0,0,134,93.9V56a6,6,0,0,0-12,0V93.9a133.5,133.5,0,0,0-18.2-22.2C85,53.1,64.2,42,48,42c-9.5,0-16.9,3.2-21.8,9.6s-7.8,17.7-6.8,32.5c.9,12.8,4.1,26.7,6.8,37.4,6,24.2,19.8,32.7,30.6,35.6A42,42,0,1,0,128,201.6a42,42,0,1,0,71.2-44.5c10.8-2.9,24.6-11.4,30.6-35.6C236.4,95.3,241.5,66.5,229.8,51.6ZM92,210a30.1,30.1,0,0,1-19.6-52.8,1,1,0,0,0,.6-.5,29.9,29.9,0,0,1,15.7-6.5,6,6,0,1,0-1.4-11.9,41.5,41.5,0,0,0-20.6,8.1c-8.9,0-22.9-4.1-28.9-27.9C34.6,105.6,26,71.3,35.6,59c2.6-3.4,6.7-5,12.4-5,12.7,0,31.3,10.3,47.4,26.3S122,114.8,122,128v52A30.1,30.1,0,0,1,92,210Zm126.2-91.5c-6,23.8-20,27.9-28.9,27.9a41.5,41.5,0,0,0-20.6-8.1,6,6,0,1,0-1.4,11.9,29.9,29.9,0,0,1,15.7,6.5,1,1,0,0,0,.6.5A30,30,0,1,1,134,180V128c0-13.2,10.2-31.5,26.6-47.7S195.3,54,208,54c5.7,0,9.8,1.6,12.4,5C230,71.3,221.4,105.6,218.2,118.5Z" }, null, -1), W = [
  P
], $ = { key: 4 }, j = /* @__PURE__ */ o("path", { d: "M231.4,50.4C227.7,45.6,220.7,40,208,40c-16.7,0-38.1,11.3-57.2,30.3A144.2,144.2,0,0,0,136,87.5V56a8,8,0,0,0-16,0V87.5a144.2,144.2,0,0,0-14.8-17.2C86.1,51.3,64.7,40,48,40c-12.7,0-19.7,5.6-23.4,10.4-6.8,8.7-12.2,24.1-.4,71.5,5.9,23.4,18.7,32.7,29.5,36.3A44.8,44.8,0,0,0,48,180a44,44,0,0,0,80,25.3A44,44,0,0,0,208,180a44.8,44.8,0,0,0-5.7-21.8c10.8-3.6,23.6-12.9,29.5-36.3C238.4,95.3,243.6,66,231.4,50.4ZM92,208a28,28,0,0,1-18.3-49.2l.7-.6a28.1,28.1,0,0,1,14.5-6,8.1,8.1,0,0,0,7.1-8.9,8,8,0,0,0-8.9-7,42.9,42.9,0,0,0-21,8.1c-8.4-.2-20.9-4.5-26.3-26.3-2.9-11.5-11.6-46.3-2.6-57.9C38.6,58.4,41.3,56,48,56c26.9,0,72,45.1,72,72v52A28.1,28.1,0,0,1,92,208Zm124.2-89.9c-5.4,21.8-17.9,26.1-26.3,26.3a42.9,42.9,0,0,0-21-8.1,8,8,0,0,0-8.9,7,8.1,8.1,0,0,0,7.1,8.9,28.1,28.1,0,0,1,14.5,6l.7.6A28,28,0,1,1,136,180V128c0-26.9,45.1-72,72-72,6.7,0,9.4,2.4,10.8,4.2C227.8,71.8,219.1,106.6,216.2,118.1Z" }, null, -1), q = [
  j
], D = { key: 5 }, F = /* @__PURE__ */ o("path", { d: "M228.3,52.8C223.7,47,216.9,44,208,44c-25.5,0-60.5,28.7-76,57.3V56a4,4,0,0,0-8,0v45.3C108.5,72.7,73.5,44,48,44c-8.9,0-15.7,3-20.3,8.8-11.1,14.3-6,42.5.4,68.2,3.6,14.4,10.4,24.7,20.3,30.5a35.7,35.7,0,0,0,11.7,4.3A40,40,0,1,0,128,197.4a40,40,0,1,0,67.9-41.6,35.7,35.7,0,0,0,11.7-4.3c9.9-5.8,16.7-16.1,20.3-30.5C234.3,95.3,239.4,67.1,228.3,52.8ZM92,212a32.1,32.1,0,0,1-20.8-56.4l.3-.2a31.8,31.8,0,0,1,16.9-7.2,4,4,0,0,0,3.6-4.4,4.1,4.1,0,0,0-4.4-3.6,40.8,40.8,0,0,0-20.2,8.2c-21.7.5-29.2-20.3-31.5-29.4C30.3,96.6,25,69.3,34.1,57.7c3-3.8,7.5-5.7,13.9-5.7,29.1,0,76,46.9,76,76v52A32.1,32.1,0,0,1,92,212Zm128.1-93c-2.3,9.1-9.8,29.9-31.5,29.4a40.8,40.8,0,0,0-20.2-8.2,4.1,4.1,0,0,0-4.4,3.6,4,4,0,0,0,3.6,4.4,31.8,31.8,0,0,1,16.9,7.2l.3.2A32,32,0,1,1,132,180V128c0-29.1,46.9-76,76-76,6.4,0,10.9,1.9,13.9,5.7C231,69.3,225.7,96.6,220.1,119Z" }, null, -1), G = [
  F
], H = {
  name: "PhButterfly"
}, K = /* @__PURE__ */ u({
  ...H,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(d) {
    const n = d, _ = a("weight", "regular"), h = a("size", "1em"), C = a("color", "currentColor"), p = a("mirrored", !1), s = i(() => n.weight ?? _), r = i(() => n.size ?? h), m = i(() => n.color ?? C), g = i(() => n.mirrored !== void 0 ? n.mirrored ? "scale(-1, 1)" : void 0 : p ? "scale(-1, 1)" : void 0);
    return (l, I) => (e(), c("svg", A({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: t(r),
      height: t(r),
      fill: t(m),
      transform: t(g)
    }, l.$attrs), [
      V(l.$slots, "default"),
      t(s) === "bold" ? (e(), c("g", M, S)) : t(s) === "duotone" ? (e(), c("g", v, x)) : t(s) === "fill" ? (e(), c("g", z, b)) : t(s) === "light" ? (e(), c("g", E, W)) : t(s) === "regular" ? (e(), c("g", $, q)) : t(s) === "thin" ? (e(), c("g", D, G)) : Z("", !0)
    ], 16, y));
  }
});
export {
  K as default
};
