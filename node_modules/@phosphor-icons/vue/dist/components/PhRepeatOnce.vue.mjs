import { defineComponent as A, inject as n, computed as i, openBlock as a, createElementBlock as o, mergeProps as H, unref as t, renderSlot as Z, createCommentVNode as V, createElementVNode as l } from "vue";
const y = ["width", "height", "fill", "transform"], L = { key: 0 }, f = /* @__PURE__ */ l("path", { d: "M20,128A76.1,76.1,0,0,1,96,52h99l-3.5-3.5a12,12,0,0,1,17-17l24,24a12,12,0,0,1,0,17l-24,24a12.1,12.1,0,0,1-17,0,12,12,0,0,1,0-17L195,76H96a52,52,0,0,0-52,52,12,12,0,0,1-24,0Zm204-12a12,12,0,0,0-12,12,52,52,0,0,1-52,52H61l3.5-3.5a12,12,0,0,0-17-17l-24,24a12,12,0,0,0,0,17l24,24a12.1,12.1,0,0,0,17,0,12,12,0,0,0,0-17L61,204h99a76.1,76.1,0,0,0,76-76A12,12,0,0,0,224,116Zm-92,48a12,12,0,0,0,12-12V104a12,12,0,0,0-5.7-10.2,11.9,11.9,0,0,0-11.7-.5l-16,8a12,12,0,0,0,9.4,22V152A12,12,0,0,0,132,164Z" }, null, -1), w = [
  f
], M = { key: 1 }, k = /* @__PURE__ */ l("path", { d: "M24,128A72.1,72.1,0,0,1,96,56H204.7L194.3,45.7a8.1,8.1,0,0,1,11.4-11.4l24,24a8.1,8.1,0,0,1,0,11.4l-24,24a8.2,8.2,0,0,1-11.4,0,8.1,8.1,0,0,1,0-11.4L204.7,72H96a56,56,0,0,0-56,56,8,8,0,0,1-16,0Zm200-8a8,8,0,0,0-8,8,56,56,0,0,1-56,56H51.3l10.4-10.3a8.1,8.1,0,0,0-11.4-11.4l-24,24a8.1,8.1,0,0,0,0,11.4l24,24a8.2,8.2,0,0,0,11.4,0,8.1,8.1,0,0,0,0-11.4L51.3,200H160a72.1,72.1,0,0,0,72-72A8,8,0,0,0,224,120Zm-92,40a8,8,0,0,0,8-8V104a7.9,7.9,0,0,0-3.8-6.8,8,8,0,0,0-7.8-.4l-16,8a8.2,8.2,0,0,0-3.6,10.8,8.1,8.1,0,0,0,10.8,3.5l4.4-2.2V152A8,8,0,0,0,132,160Z" }, null, -1), v = [
  k
], x = { key: 2 }, S = /* @__PURE__ */ l("path", { d: "M232,128a72.1,72.1,0,0,1-72,72H64v16a8,8,0,0,1-4.9,7.4,8.5,8.5,0,0,1-3.1.6,8.3,8.3,0,0,1-5.7-2.3l-24-24a8.1,8.1,0,0,1,0-11.4l24-24a8.4,8.4,0,0,1,8.8-1.7A8,8,0,0,1,64,168v16h96a56,56,0,0,0,56-56,8,8,0,0,1,16,0ZM32,136a8,8,0,0,0,8-8A56,56,0,0,1,96,72h96V88a8,8,0,0,0,4.9,7.4,8.5,8.5,0,0,0,3.1.6,8.3,8.3,0,0,0,5.7-2.3l24-24a8.1,8.1,0,0,0,0-11.4l-24-24a8.4,8.4,0,0,0-8.8-1.7A8,8,0,0,0,192,40V56H96a72.1,72.1,0,0,0-72,72A8,8,0,0,0,32,136Zm100,24a8,8,0,0,0,8-8V104a7.9,7.9,0,0,0-3.8-6.8,8,8,0,0,0-7.8-.4l-16,8a8.2,8.2,0,0,0-3.6,10.8,8.1,8.1,0,0,0,10.8,3.5l4.4-2.2V152A8,8,0,0,0,132,160Z" }, null, -1), z = [
  S
], C = { key: 3 }, B = /* @__PURE__ */ l("path", { d: "M26,128A70.1,70.1,0,0,1,96,58H209.5L195.8,44.2a5.9,5.9,0,0,1,8.4-8.4l24,24a5.8,5.8,0,0,1,0,8.4l-24,24a5.9,5.9,0,0,1-8.4-8.4L209.5,70H96a58,58,0,0,0-58,58,6,6,0,0,1-12,0Zm198-6a6,6,0,0,0-6,6,58,58,0,0,1-58,58H46.5l13.7-13.8a5.9,5.9,0,0,0-8.4-8.4l-24,24a5.8,5.8,0,0,0,0,8.4l24,24a5.9,5.9,0,0,0,8.4-8.4L46.5,198H160a70.1,70.1,0,0,0,70-70A6,6,0,0,0,224,122Zm-92,36a6,6,0,0,0,6-6V104a6.1,6.1,0,0,0-2.8-5.1,6.2,6.2,0,0,0-5.9-.3l-16,8a6,6,0,0,0,5.4,10.8l7.3-3.7V152A6,6,0,0,0,132,158Z" }, null, -1), N = [
  B
], b = { key: 4 }, E = /* @__PURE__ */ l("path", { d: "M24,128A72.1,72.1,0,0,1,96,56H204.7L194.3,45.7a8.1,8.1,0,0,1,11.4-11.4l24,24a8.1,8.1,0,0,1,0,11.4l-24,24a8.2,8.2,0,0,1-11.4,0,8.1,8.1,0,0,1,0-11.4L204.7,72H96a56,56,0,0,0-56,56,8,8,0,0,1-16,0Zm200-8a8,8,0,0,0-8,8,56,56,0,0,1-56,56H51.3l10.4-10.3a8.1,8.1,0,0,0-11.4-11.4l-24,24a8.1,8.1,0,0,0,0,11.4l24,24a8.2,8.2,0,0,0,11.4,0,8.1,8.1,0,0,0,0-11.4L51.3,200H160a72.1,72.1,0,0,0,72-72A8,8,0,0,0,224,120Zm-92,40a8,8,0,0,0,8-8V104a7.9,7.9,0,0,0-3.8-6.8,8,8,0,0,0-7.8-.4l-16,8a8.2,8.2,0,0,0-3.6,10.8,8.1,8.1,0,0,0,10.8,3.5l4.4-2.2V152A8,8,0,0,0,132,160Z" }, null, -1), P = [
  E
], W = { key: 5 }, $ = /* @__PURE__ */ l("path", { d: "M28,128A68.1,68.1,0,0,1,96,60H214.3L197.2,42.8a4,4,0,0,1,5.6-5.6l24,24a3.9,3.9,0,0,1,0,5.6l-24,24a3.9,3.9,0,0,1-5.6,0,3.9,3.9,0,0,1,0-5.6L214.3,68H96a60,60,0,0,0-60,60,4,4,0,0,1-8,0Zm196-4a4,4,0,0,0-4,4,60,60,0,0,1-60,60H41.7l17.1-17.2a4,4,0,0,0-5.6-5.6l-24,24a3.9,3.9,0,0,0,0,5.6l24,24a3.9,3.9,0,0,0,5.6,0,3.9,3.9,0,0,0,0-5.6L41.7,196H160a68.1,68.1,0,0,0,68-68A4,4,0,0,0,224,124Zm-92,32a4,4,0,0,0,4-4V104a3.9,3.9,0,0,0-1.9-3.4,4.1,4.1,0,0,0-3.9-.2l-16,8a4,4,0,0,0,3.6,7.2l10.2-5.1V152A4,4,0,0,0,132,156Z" }, null, -1), j = [
  $
], O = {
  name: "PhRepeatOnce"
}, D = /* @__PURE__ */ A({
  ...O,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(d) {
    const s = d, h = n("weight", "regular"), _ = n("size", "1em"), m = n("color", "currentColor"), p = n("mirrored", !1), e = i(() => s.weight ?? h), r = i(() => s.size ?? _), g = i(() => s.color ?? m), u = i(() => s.mirrored !== void 0 ? s.mirrored ? "scale(-1, 1)" : void 0 : p ? "scale(-1, 1)" : void 0);
    return (c, R) => (a(), o("svg", H({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: t(r),
      height: t(r),
      fill: t(g),
      transform: t(u)
    }, c.$attrs), [
      Z(c.$slots, "default"),
      t(e) === "bold" ? (a(), o("g", L, w)) : t(e) === "duotone" ? (a(), o("g", M, v)) : t(e) === "fill" ? (a(), o("g", x, z)) : t(e) === "light" ? (a(), o("g", C, N)) : t(e) === "regular" ? (a(), o("g", b, P)) : t(e) === "thin" ? (a(), o("g", W, j)) : V("", !0)
    ], 16, y));
  }
});
export {
  D as default
};
