import { defineComponent as u, inject as n, computed as L, openBlock as a, createElementBlock as t, mergeProps as v, unref as l, renderSlot as V, createCommentVNode as f, createElementVNode as e } from "vue";
const y = ["width", "height", "fill", "transform"], w = { key: 0 }, k = /* @__PURE__ */ e("path", { d: "M227.8,148.1a12,12,0,0,1-8.5,14.7l-21.2,5.7,5.7,21.2a12,12,0,0,1-8.5,14.7,11,11,0,0,1-3.1.4,12,12,0,0,1-11.6-8.9l-7.5-28L140,148.8V187l20.5,20.5a12,12,0,0,1,0,17,12.1,12.1,0,0,1-17,0L128,209l-15.5,15.5a12,12,0,0,1-17-17L116,187V148.8L82.9,167.9l-7.5,28a12,12,0,0,1-11.6,8.9,11,11,0,0,1-3.1-.4,12,12,0,0,1-8.5-14.7l5.7-21.2-21.2-5.7a12,12,0,1,1,6.2-23.2l28,7.5L104,128,70.9,108.9l-28,7.5a11,11,0,0,1-3.1.4,12,12,0,0,1-3.1-23.6l21.2-5.7L52.2,66.3a12,12,0,0,1,23.2-6.2l7.5,28L116,107.2V69L95.5,48.5a12,12,0,0,1,17-17L128,47l15.5-15.5a12,12,0,0,1,17,17L140,69v38.2l33.1-19.1,7.5-28a12,12,0,1,1,23.2,6.2l-5.7,21.2,21.2,5.7a12,12,0,0,1-3.1,23.6,11,11,0,0,1-3.1-.4l-28-7.5L152,128l33.1,19.1,28-7.5A12,12,0,0,1,227.8,148.1Z" }, null, -1), M = [
  k
], A = { key: 1 }, S = /* @__PURE__ */ e("path", { d: "M223.9,149.1a7.9,7.9,0,0,1-5.6,9.8l-25.1,6.8,6.7,25a7.9,7.9,0,0,1-5.6,9.8l-2.1.3a8,8,0,0,1-7.7-5.9l-7.9-29.6L136,141.9v46.8l21.7,21.6a8.1,8.1,0,0,1,0,11.4,8.2,8.2,0,0,1-11.4,0L128,203.3l-18.3,18.4a8.1,8.1,0,0,1-11.4-11.4L120,188.7V141.9L79.4,165.3l-7.9,29.6a8,8,0,0,1-7.7,5.9l-2.1-.3a7.9,7.9,0,0,1-5.6-9.8l6.7-25-25.1-6.8a8,8,0,1,1,4.2-15.4l29.5,7.9L112,128,71.4,104.6l-29.5,7.9a7.6,7.6,0,0,1-2.1.3,8,8,0,0,1-2.1-15.7l25.1-6.8-6.7-25a8,8,0,0,1,15.4-4.2l7.9,29.6L120,114.1V67.3L98.3,45.7a8.1,8.1,0,0,1,11.4-11.4L128,52.7l18.3-18.4a8.1,8.1,0,0,1,11.4,11.4L136,67.3v46.8l40.6-23.4,7.9-29.6a8,8,0,1,1,15.4,4.2l-6.7,25,25.1,6.8a8,8,0,0,1-2.1,15.7l-2.1-.3-29.5-7.9L144,128l40.6,23.4,29.5-7.9A7.9,7.9,0,0,1,223.9,149.1Z" }, null, -1), x = [
  S
], Z = { key: 2 }, z = /* @__PURE__ */ e("path", { d: "M223.9,149.1a7.9,7.9,0,0,1-5.6,9.8l-25.1,6.8,6.7,25a7.9,7.9,0,0,1-5.6,9.8l-2.1.3a8,8,0,0,1-7.7-5.9l-7.9-29.6L136,141.9v46.8l21.7,21.6a8.1,8.1,0,0,1,0,11.4,8.2,8.2,0,0,1-11.4,0L128,203.3l-18.3,18.4a8.1,8.1,0,0,1-11.4-11.4L120,188.7V141.9L79.4,165.3l-7.9,29.6a8,8,0,0,1-7.7,5.9l-2.1-.3a7.9,7.9,0,0,1-5.6-9.8l6.7-25-25.1-6.8a8,8,0,1,1,4.2-15.4l29.5,7.9L112,128,71.4,104.6l-29.5,7.9a7.6,7.6,0,0,1-2.1.3,8,8,0,0,1-2.1-15.7l25.1-6.8-6.7-25a8,8,0,0,1,15.4-4.2l7.9,29.6L120,114.1V67.3L98.3,45.7a8.1,8.1,0,0,1,11.4-11.4L128,52.7l18.3-18.4a8.1,8.1,0,0,1,11.4,11.4L136,67.3v46.8l40.6-23.4,7.9-29.6a8,8,0,1,1,15.4,4.2l-6.7,25,25.1,6.8a8,8,0,0,1-2.1,15.7l-2.1-.3-29.5-7.9L144,128l40.6,23.4,29.5-7.9A7.9,7.9,0,0,1,223.9,149.1Z" }, null, -1), C = [
  z
], B = { key: 3 }, N = /* @__PURE__ */ e("path", { d: "M222,149.7a5.9,5.9,0,0,1-4.2,7.3l-27,7.2,7.2,27a6,6,0,0,1-4.2,7.4l-1.6.2a6.1,6.1,0,0,1-5.8-4.5L178.3,164,134,138.4v51.1l22.2,22.3a5.9,5.9,0,1,1-8.4,8.4L128,200.5l-19.8,19.7a5.9,5.9,0,0,1-8.4-8.4L122,189.5V138.4L77.7,164l-8.1,30.3a6.1,6.1,0,0,1-5.8,4.5l-1.6-.2a6,6,0,0,1-4.2-7.4l7.2-27-27-7.2a5.9,5.9,0,0,1-4.2-7.3,6.1,6.1,0,0,1,7.3-4.3l30.4,8.2L116,128,71.7,102.4l-30.4,8.2-1.5.2a6,6,0,0,1-5.8-4.5A5.9,5.9,0,0,1,38.2,99l27-7.2L58,64.8a6,6,0,0,1,4.2-7.4,6.2,6.2,0,0,1,7.4,4.3L77.7,92,122,117.6V66.5L99.8,44.2a5.9,5.9,0,0,1,8.4-8.4L128,55.5l19.8-19.7a5.9,5.9,0,0,1,8.4,8.4L134,66.5v51.1L178.3,92l8.1-30.3a6.2,6.2,0,0,1,7.4-4.3,6,6,0,0,1,4.2,7.4l-7.2,27,27,7.2a5.9,5.9,0,0,1,4.2,7.3,6,6,0,0,1-5.8,4.5l-1.5-.2-30.4-8.2L140,128l44.3,25.6,30.4-8.2A6.1,6.1,0,0,1,222,149.7Z" }, null, -1), b = [
  N
], E = { key: 4 }, P = /* @__PURE__ */ e("path", { d: "M223.9,149.1a7.9,7.9,0,0,1-5.6,9.8l-25.1,6.8,6.7,25a7.9,7.9,0,0,1-5.6,9.8l-2.1.3a8,8,0,0,1-7.7-5.9l-7.9-29.6L136,141.9v46.8l21.7,21.6a8.1,8.1,0,0,1,0,11.4,8.2,8.2,0,0,1-11.4,0L128,203.3l-18.3,18.4a8.1,8.1,0,0,1-11.4-11.4L120,188.7V141.9L79.4,165.3l-7.9,29.6a8,8,0,0,1-7.7,5.9l-2.1-.3a7.9,7.9,0,0,1-5.6-9.8l6.7-25-25.1-6.8a8,8,0,1,1,4.2-15.4l29.5,7.9L112,128,71.4,104.6l-29.5,7.9a7.6,7.6,0,0,1-2.1.3,8,8,0,0,1-2.1-15.7l25.1-6.8-6.7-25a8,8,0,0,1,15.4-4.2l7.9,29.6L120,114.1V67.3L98.3,45.7a8.1,8.1,0,0,1,11.4-11.4L128,52.7l18.3-18.4a8.1,8.1,0,0,1,11.4,11.4L136,67.3v46.8l40.6-23.4,7.9-29.6a8,8,0,1,1,15.4,4.2l-6.7,25,25.1,6.8a8,8,0,0,1-2.1,15.7l-2.1-.3-29.5-7.9L144,128l40.6,23.4,29.5-7.9A7.9,7.9,0,0,1,223.9,149.1Z" }, null, -1), W = [
  P
], $ = { key: 5 }, j = /* @__PURE__ */ e("path", { d: "M220.1,150.2a4.1,4.1,0,0,1-2.9,4.9l-28.9,7.7,7.8,28.9a4,4,0,0,1-2.9,4.9l-1,.2a4.1,4.1,0,0,1-3.9-3L180,162.6l-48-27.7v55.4l22.8,22.9a3.9,3.9,0,0,1,0,5.6,3.9,3.9,0,0,1-5.6,0L128,197.7l-21.2,21.1a4,4,0,0,1-5.6-5.6L124,190.3V134.9L76,162.6l-8.3,31.2a4.1,4.1,0,0,1-3.9,3l-1-.2a4,4,0,0,1-2.9-4.9l7.8-28.9-28.9-7.7a4.1,4.1,0,0,1-2.9-4.9,3.9,3.9,0,0,1,4.9-2.8L72,155.7,120,128,72,100.3l-31.2,8.3-1,.2a4,4,0,0,1-1-7.9l28.9-7.7L59.9,64.3a4,4,0,0,1,2.9-4.9,3.9,3.9,0,0,1,4.9,2.8L76,93.4l48,27.7V65.7L101.2,42.8a4,4,0,0,1,5.6-5.6L128,58.3l21.2-21.1a4,4,0,1,1,5.6,5.6L132,65.7v55.4l48-27.7,8.3-31.2a3.9,3.9,0,0,1,4.9-2.8,4,4,0,0,1,2.9,4.9l-7.8,28.9,28.9,7.7a4,4,0,0,1-1,7.9l-1-.2L184,100.3,136,128l48,27.7,31.2-8.3A3.9,3.9,0,0,1,220.1,150.2Z" }, null, -1), q = [
  j
], D = {
  name: "PhSnowflake"
}, H = /* @__PURE__ */ u({
  ...D,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(d) {
    const s = d, c = n("weight", "regular"), _ = n("size", "1em"), h = n("color", "currentColor"), p = n("mirrored", !1), o = L(() => s.weight ?? c), i = L(() => s.size ?? _), g = L(() => s.color ?? h), m = L(() => s.mirrored !== void 0 ? s.mirrored ? "scale(-1, 1)" : void 0 : p ? "scale(-1, 1)" : void 0);
    return (r, F) => (a(), t("svg", v({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: l(i),
      height: l(i),
      fill: l(g),
      transform: l(m)
    }, r.$attrs), [
      V(r.$slots, "default"),
      l(o) === "bold" ? (a(), t("g", w, M)) : l(o) === "duotone" ? (a(), t("g", A, x)) : l(o) === "fill" ? (a(), t("g", Z, C)) : l(o) === "light" ? (a(), t("g", B, b)) : l(o) === "regular" ? (a(), t("g", E, W)) : l(o) === "thin" ? (a(), t("g", $, q)) : f("", !0)
    ], 16, y));
  }
});
export {
  H as default
};
