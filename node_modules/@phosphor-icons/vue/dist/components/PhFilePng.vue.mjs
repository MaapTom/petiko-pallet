import { defineComponent as H, inject as n, computed as c, openBlock as t, createElementBlock as o, mergeProps as p, unref as a, renderSlot as A, createCommentVNode as g, createElementVNode as e } from "vue";
const M = ["width", "height", "fill", "transform"], u = { key: 0 }, y = /* @__PURE__ */ e("path", { d: "M160,164v56a12,12,0,0,1-21.8,7L120,201.4V220a12,12,0,0,1-24,0V164a12,12,0,0,1,21.8-7L136,182.6V164a12,12,0,0,1,24,0ZM32,124V44A20.1,20.1,0,0,1,52,24H156a12.3,12.3,0,0,1,8.5,3.5l56,56A12.3,12.3,0,0,1,224,92v32a12,12,0,0,1-24,0V112H148a12,12,0,0,1-12-12V48H56v76a12,12,0,0,1-24,0ZM160,57V88h31ZM84,184a32.1,32.1,0,0,1-32,32H48v4a12,12,0,0,1-24,0V164a12,12,0,0,1,12-12H52A32.1,32.1,0,0,1,84,184Zm-24,0a8,8,0,0,0-8-8H48v16h4A8,8,0,0,0,60,184Zm168,2.9h-8a12,12,0,0,0-12,12,11.9,11.9,0,0,0,3.1,8A12.6,12.6,0,0,1,206,208c-7.7,0-14-7.2-14-16s6.3-16,14-16a12.8,12.8,0,0,1,7.2,2.3,12,12,0,0,0,13.6-19.8A37.1,37.1,0,0,0,206,152c-21,0-38,17.9-38,40s17,40,38,40c13.3,0,25.4-7.1,32.4-19a12.9,12.9,0,0,0,1.6-6.1v-8A12,12,0,0,0,228,186.9Z" }, null, -1), f = [
  y
], L = { key: 1 }, w = /* @__PURE__ */ e("path", {
  d: "M208,88H152V32Z",
  opacity: "0.2"
}, null, -1), k = /* @__PURE__ */ e("path", { d: "M60,160H44a8,8,0,0,0-8,8v48a8,8,0,0,0,16,0v-8h8a24,24,0,0,0,0-48Zm0,32H52V176h8a8,8,0,0,1,0,16Zm92-24v48a8,8,0,0,1-5.5,7.6,7.3,7.3,0,0,1-2.5.4,7.8,7.8,0,0,1-6.4-3.2L116,192v24a8,8,0,0,1-16,0V168a8,8,0,0,1,5.5-7.6,7.9,7.9,0,0,1,8.9,2.8L136,192V168a8,8,0,0,1,16,0Zm72,37.8a7.5,7.5,0,0,1-1.3,4.3A29.4,29.4,0,0,1,198,224c-16.5,0-30-14.4-30-32s13.5-32,30-32a28.4,28.4,0,0,1,16.6,5.4,8,8,0,0,1-9.2,13A12.6,12.6,0,0,0,198,176c-7.7,0-14,7.2-14,16s6.3,16,14,16a13.3,13.3,0,0,0,10-4.8V202h-4a8,8,0,0,1,0-16h12a8,8,0,0,1,8,8ZM200,128a8,8,0,0,0,16,0V88a7.8,7.8,0,0,0-2.4-5.7l-55.9-56A8.1,8.1,0,0,0,152,24H56A16,16,0,0,0,40,40v88a8,8,0,0,0,16,0V40h88V88a8,8,0,0,0,8,8h48ZM160,51.3,188.7,80H160Z" }, null, -1), x = [
  w,
  k
], S = { key: 2 }, z = /* @__PURE__ */ e("path", { d: "M60,160H44a8,8,0,0,0-8,8v48a8,8,0,0,0,16,0v-8h8a24,24,0,0,0,0-48Zm0,32H52V176h8a8,8,0,0,1,0,16Zm92-24v48a8,8,0,0,1-5.5,7.6,7.3,7.3,0,0,1-2.5.4,7.8,7.8,0,0,1-6.4-3.2L116,192v24a8,8,0,0,1-16,0V168a8,8,0,0,1,5.5-7.6,7.9,7.9,0,0,1,8.9,2.8L136,192V168a8,8,0,0,1,16,0Zm72,37.8a7.5,7.5,0,0,1-1.3,4.3A29.4,29.4,0,0,1,198,224c-16.5,0-30-14.4-30-32s13.5-32,30-32a28.4,28.4,0,0,1,16.6,5.4,8,8,0,0,1-9.2,13A12.6,12.6,0,0,0,198,176c-7.7,0-14,7.2-14,16s6.3,16,14,16a13.3,13.3,0,0,0,10-4.8V202h-4a8,8,0,0,1,0-16h12a8,8,0,0,1,8,8ZM48,136H208a8,8,0,0,0,8-8V88a8.1,8.1,0,0,0-2.3-5.7l-56-56A8.1,8.1,0,0,0,152,24H56A16,16,0,0,0,40,40v88A8,8,0,0,0,48,136ZM152,44l44,44H152Z" }, null, -1), C = [
  z
], B = { key: 3 }, N = /* @__PURE__ */ e("path", { d: "M60,162H44a6,6,0,0,0-6,6v48a6,6,0,0,0,12,0V206H60a22,22,0,0,0,0-44Zm0,32H50V174H60a10,10,0,0,1,0,20Zm90-26v48a6,6,0,0,1-4.1,5.7,6.3,6.3,0,0,1-1.9.3,6.1,6.1,0,0,1-4.8-2.4L114,186v30a6,6,0,0,1-12,0V168a6,6,0,0,1,10.8-3.6L138,198V168a6,6,0,0,1,12,0Zm72,37.8a5.6,5.6,0,0,1-1,3.3A27.4,27.4,0,0,1,198,222c-15.4,0-28-13.5-28-30s12.6-30,28-30a26.5,26.5,0,0,1,15.5,5,6,6,0,1,1-7,9.8A14.7,14.7,0,0,0,198,174c-8.8,0-16,8.1-16,18s7.2,18,16,18a15.5,15.5,0,0,0,12-6.1V200h-6a6,6,0,0,1,0-12h12a6,6,0,0,1,6,6ZM202,128a6,6,0,0,0,12,0V88a6,6,0,0,0-1.8-4.3l-56-55.9A5.6,5.6,0,0,0,152,26H56A14,14,0,0,0,42,40v88a6,6,0,0,0,12,0V40a2,2,0,0,1,2-2h90V88a6,6,0,0,0,6,6h50ZM158,46.5,193.5,82H158Z" }, null, -1), P = [
  N
], b = { key: 4 }, E = /* @__PURE__ */ e("path", { d: "M60,160H44a8,8,0,0,0-8,8v48a8,8,0,0,0,16,0v-8h8a24,24,0,0,0,0-48Zm0,32H52V176h8a8,8,0,0,1,0,16Zm92-24v48a8,8,0,0,1-5.5,7.6,7.3,7.3,0,0,1-2.5.4,7.8,7.8,0,0,1-6.4-3.2L116,192v24a8,8,0,0,1-16,0V168a8,8,0,0,1,5.5-7.6,7.9,7.9,0,0,1,8.9,2.8L136,192V168a8,8,0,0,1,16,0Zm72,37.8a7.5,7.5,0,0,1-1.3,4.3A29.4,29.4,0,0,1,198,224c-16.5,0-30-14.4-30-32s13.5-32,30-32a28.4,28.4,0,0,1,16.6,5.4,8,8,0,0,1-9.2,13A12.6,12.6,0,0,0,198,176c-7.7,0-14,7.2-14,16s6.3,16,14,16a13.3,13.3,0,0,0,10-4.8V202h-4a8,8,0,0,1,0-16h12a8,8,0,0,1,8,8ZM200,128a8,8,0,0,0,16,0V88a7.8,7.8,0,0,0-2.4-5.7l-55.9-56A8.1,8.1,0,0,0,152,24H56A16,16,0,0,0,40,40v88a8,8,0,0,0,16,0V40h88V88a8,8,0,0,0,8,8h48ZM160,51.3,188.7,80H160Z" }, null, -1), W = [
  E
], $ = { key: 5 }, j = /* @__PURE__ */ e("path", { d: "M60,164H44a4,4,0,0,0-4,4v48a4,4,0,0,0,8,0V204H60a20,20,0,0,0,0-40Zm0,32H48V172H60a12,12,0,0,1,0,24Zm88-28v48a4.1,4.1,0,0,1-2.7,3.8l-1.3.2a4.2,4.2,0,0,1-3.2-1.6L112,180v36a4,4,0,0,1-8,0V168a4.1,4.1,0,0,1,2.7-3.8,4,4,0,0,1,4.5,1.4L140,204V168a4,4,0,0,1,8,0Zm72,37.8a4.3,4.3,0,0,1-.6,2.2A25.7,25.7,0,0,1,198,220c-14.3,0-26-12.6-26-28s11.7-28,26-28a24.6,24.6,0,0,1,14.3,4.6,4.1,4.1,0,0,1,1,5.6,4,4,0,0,1-5.6.9A17.2,17.2,0,0,0,198,172c-9.9,0-18,9-18,20s8.1,20,18,20a17.6,17.6,0,0,0,14-7.4V198h-8a4,4,0,0,1,0-8h12a4,4,0,0,1,4,4ZM204,128a4,4,0,0,0,8,0V88.1h0a4.5,4.5,0,0,0-1.2-2.9l-56-55.9A3.6,3.6,0,0,0,152,28H56A12,12,0,0,0,44,40v88a4,4,0,0,0,8,0V40a4,4,0,0,1,4-4h92V88a4,4,0,0,0,4,4h52ZM156,41.7,198.3,84H156Z" }, null, -1), F = [
  j
], q = {
  name: "PhFilePng"
}, I = /* @__PURE__ */ H({
  ...q,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(l) {
    const h = l, d = n("weight", "regular"), V = n("size", "1em"), _ = n("color", "currentColor"), m = n("mirrored", !1), s = c(() => h.weight ?? d), i = c(() => h.size ?? V), Z = c(() => h.color ?? _), v = c(() => h.mirrored !== void 0 ? h.mirrored ? "scale(-1, 1)" : void 0 : m ? "scale(-1, 1)" : void 0);
    return (r, D) => (t(), o("svg", p({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: a(i),
      height: a(i),
      fill: a(Z),
      transform: a(v)
    }, r.$attrs), [
      A(r.$slots, "default"),
      a(s) === "bold" ? (t(), o("g", u, f)) : a(s) === "duotone" ? (t(), o("g", L, x)) : a(s) === "fill" ? (t(), o("g", S, C)) : a(s) === "light" ? (t(), o("g", B, P)) : a(s) === "regular" ? (t(), o("g", b, W)) : a(s) === "thin" ? (t(), o("g", $, F)) : g("", !0)
    ], 16, M));
  }
});
export {
  I as default
};
