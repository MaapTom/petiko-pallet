import { defineComponent as u, inject as s, computed as h, openBlock as l, createElementBlock as e, mergeProps as y, unref as t, renderSlot as f, createCommentVNode as L, createElementVNode as o } from "vue";
const V = ["width", "height", "fill", "transform"], Z = { key: 0 }, w = /* @__PURE__ */ o("path", { d: "M140,80v41.1l35.6,20.5A12.1,12.1,0,0,1,180,158a12.2,12.2,0,0,1-10.4,6,11.7,11.7,0,0,1-6-1.6l-41.6-24-.3-.2-.6-.4-.4-.3-.5-.4-.5-.4-.3-.4-.5-.5-.3-.3-.4-.6-.3-.4-.3-.5-.3-.5-.2-.5-.3-.6c0-.2-.1-.3-.1-.5a1.4,1.4,0,0,1-.2-.6l-.2-.5c0-.2-.1-.4-.1-.6s-.1-.4-.1-.6V129a1.5,1.5,0,0,1-.1-.7V80a12,12,0,0,1,24,0Zm58.7-22.7a100.1,100.1,0,0,0-141.4,0L43.8,70.7v-11a12,12,0,1,0-24,0v40c0,.4.1.8.1,1.2a.9.9,0,0,0,.1.5,2,2,0,0,0,.1.7c0,.2.1.4.1.6s.1.3.2.5.1.4.1.6a.8.8,0,0,1,.2.5l.3.5.3.6.2.5.4.5.3.5c.1.1.2.3.4.4l.7.9h.1l.8.8.5.3.5.4.5.3.5.3.5.3.5.2.6.2.5.2h.6l.6.2H71.8a12,12,0,0,0,0-24h-11L74.3,74.3a76,76,0,1,1,0,107.4,12.2,12.2,0,0,0-17,0,12,12,0,0,0,0,17A100,100,0,0,0,198.7,57.3Z" }, null, -1), k = [
  w
], H = { key: 1 }, M = /* @__PURE__ */ o("g", { opacity: "0.2" }, [
  /* @__PURE__ */ o("circle", {
    cx: "128",
    cy: "128",
    r: "88"
  })
], -1), A = /* @__PURE__ */ o("path", { d: "M128,72a8,8,0,0,0-8,8v48.7c0,.1.1.2.1.4v.4c.1.1.1.2.1.4a.4.4,0,0,1,.1.3l.2.4a.4.4,0,0,0,.1.3c0,.2.1.3.1.4l.2.3a.5.5,0,0,0,.2.4l.2.3.2.3.2.4.2.2c.1.1.2.3.4.4l.2.2.3.3.3.2.3.3.4.2h.2l41.6,24a7.1,7.1,0,0,0,4,1.1,8,8,0,0,0,4-14.9L136,123.4V80A8,8,0,0,0,128,72Z" }, null, -1), C = /* @__PURE__ */ o("path", { d: "M195.9,60.1a96.2,96.2,0,0,0-135.8,0L39.8,80.4V59.7a8,8,0,1,0-16,0v40h0a2.2,2.2,0,0,0,.1.8v.4c0,.1.1.2.1.4s.1.2.1.4a.4.4,0,0,0,.1.3c0,.2.1.3.1.4a.6.6,0,0,1,.1.4l.2.3c.1.1.1.3.2.4l.2.3c0,.1.1.2.2.4l.2.3.2.3,1.2,1.1.2.2.4.3.3.2.4.2h.3l.4.2h.3l.4.2H71.8a8,8,0,0,0,0-16H51.1L71.4,71.4a80,80,0,1,1,0,113.2,7.9,7.9,0,0,0-11.3,0,8,8,0,0,0,0,11.3A96,96,0,0,0,195.9,60.1Z" }, null, -1), x = [
  M,
  A,
  C
], S = { key: 2 }, z = /* @__PURE__ */ o("path", { d: "M136,80v43.4l37.6,21.7a8,8,0,0,1-4,14.9,7.1,7.1,0,0,1-4-1.1l-41.6-24h-.2l-.4-.2-.3-.3-.3-.2-.3-.3-.2-.2c-.2-.1-.3-.3-.4-.4l-.2-.2-.2-.4-.2-.3-.2-.3a.5.5,0,0,1-.2-.4l-.2-.3c0-.1-.1-.2-.1-.4a.4.4,0,0,1-.1-.3l-.2-.4a.4.4,0,0,0-.1-.3c0-.2,0-.3-.1-.4v-.4c0-.2-.1-.3-.1-.4V80a8,8,0,0,1,16,0Zm59.9-19.9a96.2,96.2,0,0,0-135.8,0l-8.3,8.3L37.5,54.1a8,8,0,0,0-8.7-1.8,8.2,8.2,0,0,0-5,7.4v40a8,8,0,0,0,8,8h40a8,8,0,0,0,5.7-13.6L63.1,79.7l8.3-8.3a80,80,0,1,1,0,113.2,7.9,7.9,0,0,0-11.3,0,8,8,0,0,0,0,11.3A96,96,0,0,0,195.9,60.1Z" }, null, -1), B = [
  z
], N = { key: 3 }, b = /* @__PURE__ */ o("path", { d: "M134,80v44.5l38.6,22.3a6,6,0,0,1-6,10.4l-41.6-24h-.1l-.3-.2-.3-.2h-.2l-.2-.3h-.2l-.2-.3-.2-.2-.2-.2v-.2l-.2-.3c0-.1-.1-.2-.1-.3s-.1-.1-.1-.2l-.2-.3c0-.1,0-.2-.1-.2a.8.8,0,0,0-.1-.4v-.2a.4.4,0,0,1-.1-.3v-.3a.4.4,0,0,1-.1-.3V80a6,6,0,0,1,12,0Zm60.5-18.5a94.3,94.3,0,0,0-133,0L37.8,85.2V59.7a6,6,0,1,0-12,0v40c0,.4.1.8.1,1.1h0a4.2,4.2,0,0,0,.4,1h0a4.7,4.7,0,0,0,.5,1h0l.8,1h0l.9.7h.1l1,.5h0l1.1.3H71.8a6,6,0,0,0,0-12H46.3L70,70a82,82,0,1,1,0,116,5.9,5.9,0,0,0-8.5,0,6.1,6.1,0,0,0,0,8.5,94,94,0,0,0,133-133Z" }, null, -1), E = [
  b
], P = { key: 4 }, W = /* @__PURE__ */ o("path", { d: "M136,80v43.4l37.6,21.7a8,8,0,0,1-4,14.9,7.1,7.1,0,0,1-4-1.1l-41.6-24h-.2l-.4-.3-.3-.2-.3-.2-.3-.3-.2-.2c-.2-.1-.3-.3-.4-.4l-.2-.2-.2-.4-.2-.3-.2-.3a.5.5,0,0,1-.2-.4l-.2-.3c0-.1-.1-.2-.1-.4a.4.4,0,0,1-.1-.3l-.2-.4a.4.4,0,0,0-.1-.3c0-.2,0-.3-.1-.4v-.4c0-.2-.1-.3-.1-.4V80a8,8,0,0,1,16,0Zm59.9-19.9a96.2,96.2,0,0,0-135.8,0L39.8,80.4V59.7a8,8,0,1,0-16,0v40a2.2,2.2,0,0,0,.1.8v.4c0,.1.1.2.1.4s.1.2.1.4a.4.4,0,0,0,.1.3c0,.2.1.3.1.4a.6.6,0,0,1,.1.4l.2.3c.1.1.1.3.2.4l.2.3c0,.1.1.2.2.4l.2.3.2.3,1.2,1.1.2.2.4.3.3.2.4.2h.3l.4.2h.3l.4.2H71.8a8,8,0,0,0,0-16H51.1L71.4,71.4a80,80,0,1,1,0,113.2,7.9,7.9,0,0,0-11.3,0,8,8,0,0,0,0,11.3A96,96,0,0,0,195.9,60.1Z" }, null, -1), $ = [
  W
], j = { key: 5 }, q = /* @__PURE__ */ o("path", { d: "M132,80v45.7l39.6,22.8A4,4,0,0,1,173,154a3.9,3.9,0,0,1-3.4,2,4.6,4.6,0,0,1-2-.5l-41.6-24h-.1l-.4-.3-.2-.2-.3-.3-.2-.3-.3-.3c0-.1-.1-.2-.1-.3l-.2-.4c0-.1-.1-.3-.1-.4v-.3a.9.9,0,0,1-.1-.5h0V80a4,4,0,0,1,8,0Zm61.1-17.1a92.3,92.3,0,0,0-130.2,0L35.8,90.1V59.7a4,4,0,1,0-8,0v40c0,.3.1.5.1.8v.3a.8.8,0,0,1,.1.4l.2.4.2.3.5.6h0l.6.4.3.2.4.2H71.8a4,4,0,0,0,0-8H41.5L68.6,68.6a84,84,0,1,1,0,118.8,4,4,0,0,0-5.7,0,4.2,4.2,0,0,0,0,5.7A92.1,92.1,0,0,0,193.1,62.9Z" }, null, -1), D = [
  q
], F = {
  name: "PhClockCounterClockwise"
}, J = /* @__PURE__ */ u({
  ...F,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(r) {
    const c = r, d = s("weight", "regular"), _ = s("size", "1em"), v = s("color", "currentColor"), p = s("mirrored", !1), a = h(() => c.weight ?? d), n = h(() => c.size ?? _), m = h(() => c.color ?? v), g = h(() => c.mirrored !== void 0 ? c.mirrored ? "scale(-1, 1)" : void 0 : p ? "scale(-1, 1)" : void 0);
    return (i, G) => (l(), e("svg", y({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: t(n),
      height: t(n),
      fill: t(m),
      transform: t(g)
    }, i.$attrs), [
      f(i.$slots, "default"),
      t(a) === "bold" ? (l(), e("g", Z, k)) : t(a) === "duotone" ? (l(), e("g", H, x)) : t(a) === "fill" ? (l(), e("g", S, B)) : t(a) === "light" ? (l(), e("g", N, E)) : t(a) === "regular" ? (l(), e("g", P, $)) : t(a) === "thin" ? (l(), e("g", j, D)) : L("", !0)
    ], 16, V));
  }
});
export {
  J as default
};
