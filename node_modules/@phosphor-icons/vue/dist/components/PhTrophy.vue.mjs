import { defineComponent as m, inject as n, computed as c, openBlock as o, createElementBlock as a, mergeProps as Z, unref as t, renderSlot as g, createCommentVNode as v, createElementVNode as e } from "vue";
const u = ["width", "height", "fill", "transform"], M = { key: 0 }, y = /* @__PURE__ */ e("path", { d: "M232,60H212V56a20.1,20.1,0,0,0-20-20H64A20.1,20.1,0,0,0,44,56v4H23.9a20,20,0,0,0-20,20V96a44,44,0,0,0,44,44H49c10.5,28.9,36.1,50.6,67,55.1V212H96a12,12,0,0,0,0,24h64a12,12,0,0,0,0-24H140V195.1A83.9,83.9,0,0,0,207.2,140h.8a44,44,0,0,0,44-44V80A20.1,20.1,0,0,0,232,60ZM27.9,96V84H44v27.1c0,1.5,0,3,.1,4.5A20,20,0,0,1,27.9,96Zm99.6,76C94.7,171.8,68,144.4,68,111.1V60H188v52a59.9,59.9,0,0,1-60.5,60ZM228,96a20,20,0,0,1-16.1,19.6c.1-1.2.1-2.4.1-3.6V84h16Z" }, null, -1), f = [
  y
], w = { key: 1 }, C = /* @__PURE__ */ e("path", {
  d: "M200,56v56a72,72,0,0,1-72.5,72C87.8,183.7,56,150.8,56,111.1V56a8,8,0,0,1,8-8H192A8,8,0,0,1,200,56Z",
  opacity: "0.2"
}, null, -1), k = /* @__PURE__ */ e("path", { d: "M232,64H208V56a16,16,0,0,0-16-16H64A16,16,0,0,0,48,56v8H23.9a16,16,0,0,0-16,16V96a40,40,0,0,0,40,40h4c9.7,29.9,36.2,52.3,68.1,55.6V216H96a8,8,0,0,0,0,16h64a8,8,0,0,0,0-16H136V191.6A79.7,79.7,0,0,0,204.3,136H208a40,40,0,0,0,40-40V80A16,16,0,0,0,232,64ZM47.9,120a24,24,0,0,1-24-24V80H48v31.1a86.9,86.9,0,0,0,.5,8.9Zm125.2,37.4A63.6,63.6,0,0,1,128,176h-.5c-35-.3-63.5-29.4-63.5-64.9V56H192v56A63.4,63.4,0,0,1,173.1,157.4ZM232,96a24.1,24.1,0,0,1-24,24h-.4a70.3,70.3,0,0,0,.4-8V80h24Z" }, null, -1), x = [
  C,
  k
], S = { key: 2 }, z = /* @__PURE__ */ e("path", { d: "M232,64H208V56a16,16,0,0,0-16-16H64A16,16,0,0,0,48,56v8H23.9a16,16,0,0,0-16,16V96a40,40,0,0,0,40,40h4c9.7,29.9,36.2,52.3,68.1,55.6V216H96a8,8,0,0,0,0,16h64a8,8,0,0,0,0-16H136V191.6A79.7,79.7,0,0,0,204.3,136H208a40,40,0,0,0,40-40V80A16,16,0,0,0,232,64ZM47.9,120a24,24,0,0,1-24-24V80H48v31.1a86.9,86.9,0,0,0,.5,8.9ZM232,96a24.1,24.1,0,0,1-24,24h-.4a70.3,70.3,0,0,0,.4-8V80h24Z" }, null, -1), B = [
  z
], N = { key: 3 }, b = /* @__PURE__ */ e("path", { d: "M232,66H206V56a14,14,0,0,0-14-14H64A14,14,0,0,0,50,56V66H23.9a14,14,0,0,0-14,14V96a38,38,0,0,0,38,38h5.5c9.2,30.5,36.2,53.2,68.6,55.7V218H96a6,6,0,0,0,0,12h64a6,6,0,0,0,0-12H134V189.8a77,77,0,0,0,48.9-22.4,78.7,78.7,0,0,0,20-33.4H208a38,38,0,0,0,38-38V80A14,14,0,0,0,232,66ZM47.9,122a26,26,0,0,1-26-26V80a2,2,0,0,1,2-2H50v33.1a78.8,78.8,0,0,0,.8,10.9Zm126.6,36.8A65.6,65.6,0,0,1,128,178h-.5C91.4,177.7,62,147.7,62,111.1V56a2,2,0,0,1,2-2H192a2,2,0,0,1,2,2v56A65.2,65.2,0,0,1,174.5,158.8ZM234,96a26.1,26.1,0,0,1-26,26h-2.7a66.3,66.3,0,0,0,.7-10V78h26a2,2,0,0,1,2,2Z" }, null, -1), E = [
  b
], P = { key: 4 }, W = /* @__PURE__ */ e("path", { d: "M232,64H208V56a16,16,0,0,0-16-16H64A16,16,0,0,0,48,56v8H23.9a16,16,0,0,0-16,16V96a40,40,0,0,0,40,40h4c9.7,29.9,36.2,52.3,68.1,55.6V216H96a8,8,0,0,0,0,16h64a8,8,0,0,0,0-16H136V191.6A79.7,79.7,0,0,0,204.3,136H208a40,40,0,0,0,40-40V80A16,16,0,0,0,232,64ZM47.9,120a24,24,0,0,1-24-24V80H48v31.1a86.9,86.9,0,0,0,.5,8.9Zm125.2,37.4A63.6,63.6,0,0,1,128,176h-.5c-35-.3-63.5-29.4-63.5-64.9V56H192v56A63.4,63.4,0,0,1,173.1,157.4ZM232,96a24.1,24.1,0,0,1-24,24h-.4a70.3,70.3,0,0,0,.4-8V80h24Z" }, null, -1), $ = [
  W
], j = { key: 5 }, T = /* @__PURE__ */ e("path", { d: "M232,68H204V56a12,12,0,0,0-12-12H64A12,12,0,0,0,52,56V68H23.9a12,12,0,0,0-12,12V96a36,36,0,0,0,36,36h7c8.7,31,36.2,54.1,69.1,55.9V220H96a4,4,0,0,0,0,8h64a4,4,0,0,0,0-8H132V187.9a75.5,75.5,0,0,0,49.5-22A74.1,74.1,0,0,0,201.3,132H208a36,36,0,0,0,36-36V80A12,12,0,0,0,232,68ZM47.9,124a28,28,0,0,1-28-28V80a4,4,0,0,1,4-4H52v35.1A78.3,78.3,0,0,0,53.1,124Zm128,36.3A68.1,68.1,0,0,1,127.5,180C90.3,179.7,60,148.8,60,111.1V56a4,4,0,0,1,4-4H192a4,4,0,0,1,4,4v56A67.4,67.4,0,0,1,175.9,160.3ZM236,96a28.1,28.1,0,0,1-28,28h-4.9a78.8,78.8,0,0,0,.9-12V76h28a4,4,0,0,1,4,4Z" }, null, -1), q = [
  T
], D = {
  name: "PhTrophy"
}, I = /* @__PURE__ */ m({
  ...D,
  props: {
    weight: {
      type: String
    },
    size: {
      type: [String, Number]
    },
    color: {
      type: String
    },
    mirrored: {
      type: Boolean
    }
  },
  setup(d) {
    const h = d, V = n("weight", "regular"), l = n("size", "1em"), H = n("color", "currentColor"), _ = n("mirrored", !1), s = c(() => h.weight ?? V), i = c(() => h.size ?? l), p = c(() => h.color ?? H), A = c(() => h.mirrored !== void 0 ? h.mirrored ? "scale(-1, 1)" : void 0 : _ ? "scale(-1, 1)" : void 0);
    return (r, F) => (o(), a("svg", Z({
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 256 256",
      width: t(i),
      height: t(i),
      fill: t(p),
      transform: t(A)
    }, r.$attrs), [
      g(r.$slots, "default"),
      t(s) === "bold" ? (o(), a("g", M, f)) : t(s) === "duotone" ? (o(), a("g", w, x)) : t(s) === "fill" ? (o(), a("g", S, B)) : t(s) === "light" ? (o(), a("g", N, E)) : t(s) === "regular" ? (o(), a("g", P, $)) : t(s) === "thin" ? (o(), a("g", j, q)) : v("", !0)
    ], 16, u));
  }
});
export {
  I as default
};
